...
if app_mode == "Curls" and not about:
    ...
    if "my_input_value2" not in st.session_state:
        st.session_state.my_input_value2 = ""
...
if app_mode == "Squats" and not about:
    ...
    if body_weight:
        send_report = st.button("Send Report")
        calories_burned, duration = get_calories_burned('squats', 2, count)
        if send_report:
            report_sender("AAbdusalam01@my.fisk.edu", duration, calories_burned)
